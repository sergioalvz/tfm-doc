...
Para entender los siguientes resultados, será necesario explicar dos conceptos clave que se repetirán a lo largo de las diferentes tablas resumen. El concepto de *acierto* se puede entender como el número de predicciones que coinciden con los resultados reales sobre el total de las predicciones realizadas.

Por tanto, si tenemos un total de 20 predicciones y 18 de las mismas han coincidido con su valor real, se consideraría que el porcentaje de acierto sería:

----
(18/20) * 100 = 90% de acierto
----
...
Una característica que permite manejar el grado de exhaustividad de cada modelo es el valor que se determine como *umbral de acierto*.
...
Ese valor que determinemos como umbral permitirá que más o menos valores sean etiquetados por el clasificador como locales y, por tanto, que más o menos tuits locales puedan ser potencialmente etiquetados. Sin embargo, cuanto más bajo pongamos el umbral de acierto, más baja será la precisión y por tanto se encontrará más ruido en los resultados finales.

Para los siguientes ejemplos se ha tomado como umbral el valor *0.5*, el cual demuestra tener, de manera empírica, un buen comportamiento en la relación acierto - exhaustividad.
